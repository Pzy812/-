<template>
  <div class="case2-phase-diagram">
    <el-card class="mb-4">
      <template #header>
        <div class="card-header">
          <span class="title-large">五元高熵合金相图计算</span>
          <el-button
            type="primary"
            size="default"
      ...
      <el-form :model="formData" label-width="140px" size="default" label-position="left">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Ni元素摩尔分数" prop="niFraction">
              <el-input-number
                v-model="formData.niFraction"
                :min="0.001"
                :max="0.999"
                :precision="3"
                :step="0.01"
          ...
          <el-col :span="12">
            <el-form-item label="Al元素摩尔分数" prop="alFraction">
              <el-input-number
                v-model="formData.alFraction"
                :min="0.001"
                :max="0.999"
                :precision="3"
                :step="0.01"
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Co元素摩尔分数" prop="coFraction">
              <el-input-number
                v-model="formData.coFraction"
                :min="0.001"
                :max="0.999"
                :precision="3"
                :step="0.01"
          ...
          <el-col :span="12">
            <el-form-item label="Y轴最低值">
              <el-input-number
                ...
                style="width: 100%"
                controls-position="right"
                placeholder="请输入最低值(0-0.3)"
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Y轴最高值">
              <el-input-number
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const validateForm = () => {
      // 验证各元素摩尔分数
      if (formData.value.niFraction <= 0 || formData.value.niFraction >= 1) {
        ElMessage.warning("Ni元素摩尔分数必须大于0且小于1");
        return false;
      }

      if (formData.value.alFraction <= 0 || formData.value.alFraction >= 1) {
        ElMessage.warning("Al元素摩尔分数必须大于0且小于1");
        return false;
      }

      if (formData.value.coFraction <= 0 || formData.value.coFraction >= 1) {
        ElMessage.warning("Co元素摩尔分数必须大于0且小于1");
        return false;
      }
      ...
      // 验证总摩尔分数
      const totalFraction = formData.value.niFraction + formData.value.alFraction +
                           formData.value.coFraction + formData.value.crFraction;
      if (totalFraction >= 1) {
        ElMessage.warning("各元素摩尔分数之和必须小于1");
        return false;
      }

      // 验证Y轴范围
      if (formData.value.yLow < 0 || formData.value.yLow > 1) {
        ElMessage.warning("Y轴最低值必须在0-1范围内");
        return false;
      }

      if (formData.value.yHigh < 0 || formData.value.yHigh > 1) {
        ElMessage.warning("Y轴最高值必须在0-1范围内");
        return false;
      }

      if (formData.value.yLow >= formData.value.yHigh) {
        ElMessage.warning("Y轴最低值必须小于最高值");
        return false;
      }
    ...
    const exportResult = () => {
      if (calculationResult.value) {
        ...
        if (calculationResult.value.text) {
          ...
          a.href = url;
          a.download = `五元高熵合金计算结果_${new Date().getTime()}.txt`;
          a.click();
        ...
        if (calculationResult.value.imageUrl) {
          ...
          link.download = `五元高熵合金相图_${new Date().getTime()}.png`;
    ...
    const runCalculation = async () => {
      ...
      // 准备请求参数（移除数据库参数）
      const requestData = {
        calculationType: "case2",
        temperature: formData.value.temperature,
        pressure: formData.value.pressure,
        niFraction: formData.value.niFraction,
        alFraction: formData.value.alFraction,
        coFraction: formData.value.coFraction,
      ...
      try {
        ...
        const response = await fetch('http://58.199.161.154:8000/case2_calculate', {
          method: 'POST',
          headers: {
        ...
        emit('case2-results', calculationResult.value);
